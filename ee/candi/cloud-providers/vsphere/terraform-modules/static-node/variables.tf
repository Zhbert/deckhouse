...
locals {
  ...
  use_nested_resource_pools = lookup(var.providerClusterConfiguration, "useNestedResourcePools", true)
  base_resource_pool    = trim(lookup(var.providerClusterConfiguration, "baseResourcePool", ""), "/")
  default_resource_pool = local.use_nested_resource_pools == true ? join("/", local.base_resource_pool != "" ? [local.base_resource_pool, local.prefix] : [local.prefix]) : ""
